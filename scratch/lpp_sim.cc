  ...
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      ...
      clientApps.Start (MilliSeconds (1+rand()%numberOfNodes));
    }
}
...
int main (int argc, char *argv[])
{
    ...
    setPathLossModel(lteHelper, "ns3::OhBuildingsPropagationLossModel");
...
// we install the IP stack on the remote hosts
 ...
 for (uint32_t i = 0; i < remoteHostContainer.GetN(); ++i)
  ...
  remoteHostStaticRouting->AddNetworkRouteTo (Ipv4Address ("7.0.0.0"), Ipv4Mask ("255.0.0.0"), 1);
    ...
    Ptr<GridBuildingAllocator> g = CreateObject<GridBuildingAllocator> ();
    setGridBuildings(g, 10, 9);
    
    MobilityHelper mobility;
  // Install Mobility Model
  mobility.SetMobilityModel("ns3::GaussMarkovMobilityModel", "Bounds", BoxValue(Box(0,200,0,50,0,30)));
  double   y = 15.0, z = 1.5; 
  Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator> ();
  Ptr<UniformRandomVariable> random = CreateObject<UniformRandomVariable> ();
  listPosition(positionAlloc, random, ueNodes.GetN(), 200,50,30);
  ...
  for (uint32_t i = 0; i < enbNodes.GetN()-1; ++i)
    {
      double xpos = distance*(i+1);
      Ptr<MobilityModel> mm0 = enbNodes.Get (i)->GetObject<ConstantPositionMobilityModel> ();
       mm0->SetPosition (Vector(xpos , y, z));
       ++i;
       mm0 = enbNodes.Get (i)->GetObject<ConstantPositionMobilityModel> ();
       mm0->SetPosition (Vector(xpos , y*2, z));
    }
  ...
  BuildingsHelper::MakeMobilityModelConsistent ();
    //configure antenna before creating enbNet devices. 
/*
...
// assign IP address to UEs
  ...
  responseTime = new uint32_t[numberOfNodes+2];
  web_size = new uint32_t[numberOfNodes+2];
  httpDelay = new uint32_t[numberOfNodes+2];
  num_httpDelay = new uint32_t[numberOfNodes+2];
  throughput = new double[numberOfNodes+2];
  num_pages = new uint32_t[numberOfNodes+2];
  ...
  ApplicationContainer serverApps;
  Ptr<UniformRandomVariable> startTimeSeconds = CreateObject<UniformRandomVariable> ();
  Ptr<Ipv4> lpp_ip = remoteHostContainer.Get(1)->GetObject<Ipv4>();
  Ipv4Address lpp_addr = lpp_ip->GetAddress(1,0).GetLocal();
   
  LppServerHelper serverHelper (lpp_addr);
  serverApps = serverHelper.Install (remoteHostContainer.Get(1));
  //random_data(s, rand() % MAX_DATA);  
  //serverHelper.SetFill(serverApps.Get(0), s);
