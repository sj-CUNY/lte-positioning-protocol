 ...
 */
class LppClient : public Application 
{
public:
  /**
  ...
  typedef Callback<void, Ptr<Node> > ConnectCback;
  virtual void SetHandleConnect(ConnectCback cback);
  typedef Callback<void, Ptr<Packet> ,uint32_t, Ptr<Node> > CertificateCback;
  virtual void SetHandleCertificate(CertificateCback cback);
  void restart();
  void SetCallbacks();
protected:
...
private:
  ...
  void HandlePacket(Ptr<Packet> p, Ptr<Socket> socket, Address from);
  ...
  ConnectCback m_rqCertificate;
  CertificateCback m_rxCertificate; 
