...
public:
  ...
  typedef void (*TimeSizeCallback)( Time t,  uint32_t size,  uint32_t addr);
...
private:
  ...
  ns3::TracedCallback<const Time &, const uint32_t &>  m_rxRttTrace;
  ns3::TracedCallback<const Time &,const  uint32_t &, const uint32_t & > m_WebpageCompleted;
