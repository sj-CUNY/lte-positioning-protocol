...
ThreeGppHttpClient::ThreeGppHttpClient ()
  ...
  m_webResponseTs(MilliSeconds(0)),
  m_webSize(0),
...
ThreeGppHttpClient::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::ThreeGppHttpClient")
    ...
    .AddTraceSource ("RxEmbeddedObject",
                     ...
                     "ns3::ThreeGppHttpClient::TracedCallback")
    .AddTraceSource ("Response",
                     "Trace fired upon every web page completion.",
                     MakeTraceSourceAccessor (&ThreeGppHttpClient::m_WebpageCompleted), 
                     "ns3::ThreeGppHttpClient::TimeSizeCallback")
    .AddTraceSource ("Rx",
      ...
      if (m_objectBytesToBeReceived > 0)
      ...
      else
        {
          ...
          if (!m_objectClientTs.IsZero ())
            {
              m_rxRttTrace (Simulator::Now () - m_objectClientTs, GetNode()->GetId());
      ...
      else
        {
          ...
          if (m_embeddedObjectsToBeRequested > 0)
          ...
          else
            {
              ...
              m_WebpageCompleted(Simulator::Now () - m_webResponseTs,m_webSize, GetNode()->GetId());
              m_webResponseTs = Time(MilliSeconds(0));
              m_webSize = 0;
              EnterReadingTime ();
            }
