 ...
 */
class LppServerHelper
{
public:
  /**
   * Create LppServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  LppServerHelper (const Address &address);
  ...
  /**
   * Create a LppServerApplication on the specified Node.
   *
  ...
  /**
   * Create a LppServerApplication on specified node
   *
...
private:
  /**
   * Install an ns3::LppServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an LppServer will be installed.
 ...
 */
class LppClientHelper
{
public:
  /**
   * Create LppClientHelper which will make life easier for people trying
   ...
   */
  LppClientHelper (Address ip, uint16_t port);
  /**
   * Create LppClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
  ...
  /**
   * Given a pointer to a LppClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be LppClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the LppClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the LppClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
   ...
   */
  ApplicationContainer Install (NodeContainer c) const;
  void restart(Ptr<Application> app);
  void CertificateReqTrace(Ptr<Application> app , Callback<void, Ptr<Node> >   cback) ;  
  void CertificateRxTrace(Ptr<Application> app , Callback<void, Ptr<Packet>, uint32_t, Ptr<Node> >  cback) ;  
private:
  /**
   * Install an ns3::LppClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an LppClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
